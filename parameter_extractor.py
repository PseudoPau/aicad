...
Lightweight extractor that attempts to parse a structured JSON block from the
image analysis description produced by the refined prompt. If parsing fails,
falls back to a conservative default/template similar to previous behavior.
"""
import json
import re
...
def extract_from_image_description(description: str) -> Dict[str, Any]:
    ...
    if not parsed:
        # fallback to previous mock/template
        return {
    ...
    # Map parsed JSON to internal parameter structure
