...
### 1.2 核心价值
- **自动化建模**: 图片输入 → AI分析 → 参数提取 → CAD生成（<30s）
...
| F002 | AI视觉分析 | P0 | GPT-4o/Vision，响应时间<15s，JSON格式输出 |
| F003 | 参数提取与验证 | P0 | 提取准确率≥85%，缺失参数自动补全 |
...
### 6.1 技术栈限制
- **CAD引擎**: CadQuery (Python) - 必须支持参数化建模
- **AI模型**: OpenAI GPT-4o/Vision - 必须支持多模态输入
